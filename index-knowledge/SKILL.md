...
description: Generate hierarchical AGENTS.md + CLAUDE.md knowledge base for a codebase. Creates root + complexity-scored subdirectory documentation with both file types.
---

# index-knowledge

Generate hierarchical AGENTS.md + CLAUDE.md files. Root + complexity-scored subdirectories.
...
1. **Discovery + Analysis** (concurrent)
   ...
   - Main session: bash structure + LSP codemap + read existing AGENTS.md + CLAUDE.md
2. **Score & Decide** - Determine AGENTS.md + CLAUDE.md locations from merged findings
3. **Generate** - Root first, then subdirs in parallel (both files per location)
...
#### 2. Read Existing AGENTS.md + CLAUDE.md
```
For each existing file found (AGENTS.md and CLAUDE.md):
  Read(filePath=file)
...
### Root AGENTS.md + CLAUDE.md (Full Treatment)
...
Write identical content to both `./AGENTS.md` and `./CLAUDE.md`.

### Subdirectory AGENTS.md + CLAUDE.md (Parallel)
...
Task(
  description="AGENTS.md + CLAUDE.md for src/hooks",
  subagent_type="general",
  prompt="Generate AGENTS.md + CLAUDE.md for: src/hooks
    - Reason: high complexity
    - 30-80 lines max
    - NEVER repeat parent content
    - Sections: OVERVIEW (1 line), STRUCTURE (if >5 subdirs), WHERE TO LOOK, CONVENTIONS (if different), ANTI-PATTERNS
    - Write identical content to BOTH src/hooks/AGENTS.md AND src/hooks/CLAUDE.md"
)

Task(
  description="AGENTS.md + CLAUDE.md for src/api",
  subagent_type="general",
  prompt="Generate AGENTS.md + CLAUDE.md for: src/api
    - Reason: distinct domain
    - 30-80 lines max
    - NEVER repeat parent content
    - Sections: OVERVIEW (1 line), STRUCTURE (if >5 subdirs), WHERE TO LOOK, CONVENTIONS (if different), ANTI-PATTERNS
    - Write identical content to BOTH src/api/AGENTS.md AND src/api/CLAUDE.md"
)
// ... one Task per DOCS_LOCATIONS entry
```
...
For each location, review AGENTS.md:
- Remove generic advice
- Remove parent duplicates
- Trim to size limits
- Verify telegraphic style
- Apply all corrections identically to CLAUDE.md at the same path
...
Dirs Analyzed: {N}
Locations Generated: {N}
Files Written: {N} (AGENTS.md + CLAUDE.md per location)
...
- **Over-documenting**: Not every dir needs AGENTS.md + CLAUDE.md
- **Redundancy**: Child never repeats parent
- **Generic content**: Remove anything that applies to ALL projects
- **Verbose style**: Telegraphic or die
- **File mismatch**: AGENTS.md and CLAUDE.md at the same path must have identical content
